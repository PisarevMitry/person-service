...
public interface PersonDataRepository {
    ...
    @Insert("insert into person_data (last_name, first_name, birth_dt, age, sex, contact_id, medical_card_id) " + "values(#{lastName}, #{first_name}, #{birthDt}, #{age}, #{sex}, #{contact_id}, #{medical_card_id})")
    @Options(useGeneratedKeys = true, keyProperty = "id", keyColumn = "id")
    int insert(PersonDataEntity personDataEntity);

    @Insert("insert into person_data (last_name, first_name, birth_dt, email, password)" + "values( #{lastName},#{firstName}, #{birthDt}, #{email}, #{password})")
